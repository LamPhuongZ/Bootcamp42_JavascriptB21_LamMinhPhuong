...
function validate() {
    ...
    else if (!/(^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂẾưăạảấầẩẫậắằẳẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s\W|_]+$)/.test(name)) {
        isValid = false;
    ...
    } else if (!/(?=.{6,10})((?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^\w\s])).*$/.test(password)) {
        isValid = false;
        getElement("#tbMatKhau").innerHTML = "Mật khẩu không hợp lệ.  Cần 6-10 ký tự (chứa ít nhất 1 ký tự số, 1 ký tự in hoa, 1 ký tự đặc biệt)";
    } else {
        getElement("#tbMatKhau").innerHTML = "";
    }
    ...
    else if (!/^(((0[13-9]|1[012])[\/]?(0[1-9]|[12][0-9]|30)|(0[13578]|1[02])[-/]?31|02[-/]?(0[1-9]|1[0-9]|2[0-8]))[-/]?[0-9]{4}|02[-/]?29[-/]?([0-9]{2}(([2468][048]|[02468][48])|[13579][26])|([13579][26]|[02468][048]|0[0-9]|1[0-6])00))$/.test(datePicker)) {
        isValid = false;
    ...
    if (!basicSalary.trim()) {
        ...
        isValid = false;
        getElement("#tbLuongCB").innerHTML = "Lương cơ bản phải từ 1.000.000 -> 20.000.000";
    }
    else {
